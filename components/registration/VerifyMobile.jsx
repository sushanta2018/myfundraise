...
export default function VerifyMobile({page, setPage}) {
  ...
  return (
    <>
      ...
      <h2 className="text-[24px] font-bold leading-[26px] text-primary">{t("yourAccountTitle")}</h2>
      <div className="mt-[32px] grow">
        <input class="border-0 border-b h-[40px] py-[8px] placeholder-secondary text-dark w-full form-control text-[16px]" placeholder={t("firstName")} type="text"/>
        <input class="border-0 border-b h-[40px] py-[8px] placeholder-secondary text-dark w-full form-control text-[16px] mt-[32px]" placeholder={t("lastName")} type="email"/>
        <input class="border-0 border-b h-[40px] py-[8px] placeholder-secondary text-dark w-full form-control text-[16px] mt-[32px]" placeholder={t("mobileNumber")} type="text"/>
        <div className="mt-[32px] relative pl-[30px]">
          ...
          <p>{t("checked")} <Link className="text-ternary" href="#">Terms and conditions</Link> and <Link className="text-ternary" href="#">Privacy Policy.</Link></p>
        </div>
      </div>
      <div className="flex mt-[20px]">
        <button type="button" className="btn-hover py-[10px] px-[24px] rounded-[40px] text-[16px] font-bold bg-ternary text-white flex items-center" onClick={() => { 
          setPage(page + 1);
          }}>{t("verifyMobileNumber")} <svg className="ml-[10px]" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
